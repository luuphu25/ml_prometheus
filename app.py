...
def job(current_time):
    ...
    prom = Prometheus(url=url, data_chunk=chunk_size, stored_data=data_size)
...
def metrics():
    ...
    for metadata in predictions_dict_prophet:
        ...
        metric = (Prometheus(url=url).get_current_metric_value(metric_name, temp_current_metric_metadata_dict))
        ...
        print('#### Debug -------------------- ###')
        print(predictions_dict_prophet[metadata]['yhat'][index_prophet])
    ...
    return Response(generate_latest(REGISTRY), content_type='text; charset=utf-8')
    #return Response(generate_latest(REGISTRY), mimitype="text/plain")
